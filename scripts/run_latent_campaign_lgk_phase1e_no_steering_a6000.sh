...
LOG_PATH="${LOG_PATH:-/tmp/latent_campaign_lgk_phase1e_no_steering.log}"

echo "[latent-phase1e] gpu_uuid=${GPU_UUID} seeds=${SEED_START}-${SEED_END} log=${LOG_PATH}"
...
CUDA_VISIBLE_DEVICES="${GPU_UUID}" UV_CACHE_DIR=.uv_cache \
  uv run python src/prospero/runners/run_latent_campaign.py \
    ...
    --steering-scalars 0.0 \
    --combo-chunk-size 4 \
    > "${LOG_PATH}" 2>&1

echo "[latent-phase1e] done"
