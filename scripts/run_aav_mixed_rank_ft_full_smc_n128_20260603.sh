...
BASE="${BASE:-outputs/aav_zero_shot_evodiff_ft_rank_mixed_fullsmc_k4_n128_kl2_trace_batch64_20260603}"
TASK="${TASK:-AAV}"
...
echo "=== AAV mixed rank FT full-SMC-vocab n128 started $(date -Is) ==="
echo "BASE=${BASE}"
echo "CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
echo "N_QUERIES=${N_QUERIES} SEEDS=${SEEDS}"

out_dir="${BASE}/n_samples_${N_QUERIES}/mixed_explore_exploit_full_smc"
mkdir -p "${out_dir}/${TASK}"
for seed in ${SEEDS}; do
  save_path="${out_dir}/${TASK}/seed_${seed}.pkl"
  if seed_complete "${save_path}"; then
    echo "--- skip completed full-SMC rank seed=${seed}: ${save_path} ---"
    continue
  fi
  echo "--- run full-SMC rank n_queries=${N_QUERIES} seed=${seed} $(date -Is) ---"
...
echo "=== AAV mixed rank FT full-SMC-vocab n128 finished $(date -Is) ==="
