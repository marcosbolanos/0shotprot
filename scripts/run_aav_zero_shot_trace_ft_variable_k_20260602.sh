...
BASE="${BASE:-outputs/aav_zero_shot_evodiff_ft_k4_variable_k_kl2_trace_20260602}"
TASK="${TASK:-AAV}"
...
echo "=== traced AAV FT zero-shot run started $(date -Is) ==="
echo "BASE=${BASE}"
...
for n_queries in ${BUDGETS}; do
  ...
  echo "=== FT n_queries=${n_queries} seed_grow started $(date -Is) ==="
  for seed in ${SEEDS}; do
    save_path="${out_dir}/${TASK}/seed_${seed}.pkl"
    if seed_complete "${save_path}"; then
      echo "--- skip completed FT n_queries=${n_queries} seed=${seed}: ${save_path} ---"
      continue
    fi
    echo "--- run FT n_queries=${n_queries} seed=${seed} $(date -Is) ---"
  ...
  echo "=== FT n_queries=${n_queries} seed_grow finished $(date -Is) ==="
done

echo "=== traced AAV FT zero-shot run finished $(date -Is) ==="
