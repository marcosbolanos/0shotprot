...
LOG_PATH="${LOG_PATH:-/tmp/latent_campaign_lgk_combo_topfeat30_lambda01.log}"

echo "[latent-combo] phase=phase_combo_topfeat30_lambda_0p1_10seed gpu_uuid=${GPU_UUID} seeds=${SEED_START}-${SEED_END}"
...
CUDA_VISIBLE_DEVICES="${GPU_UUID}" UV_CACHE_DIR=.uv_cache \
  uv run python src/prospero/runners/run_latent_campaign.py \
    ...
    --task LGK \
    --phase phase_combo_topfeat30_lambda_0p1_10seed \
