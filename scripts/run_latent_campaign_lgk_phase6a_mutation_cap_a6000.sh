...
declare -a CAPS=("2" "4" "6" "8")

for CAP in "${CAPS[@]}"; do
  PHASE="phase6a_maxmut_${CAP}"
  LOG_PATH="${LOG_PREFIX}_maxmut_${CAP}.log"
  echo "[latent-phase6a] phase=${PHASE} max_mutations=${CAP} gpu_uuid=${GPU_UUID} seeds=${SEED_START}-${SEED_END}"
