...
BASE="${BASE:-outputs/aav_zero_shot_evodiff_ft_advantage_k4_variable_k_kl2_trace_batch64_20260602}"
TASK="${TASK:-AAV}"
...
echo "=== traced AAV standardized-advantage FT run started $(date -Is) ==="
echo "BASE=${BASE}"
...
for n_queries in ${BUDGETS}; do
  ...
  echo "=== advantage FT n_queries=${n_queries} seed_grow started $(date -Is) ==="
  for seed in ${SEEDS}; do
    save_path="${out_dir}/${TASK}/seed_${seed}.pkl"
    if seed_complete "${save_path}"; then
      echo "--- skip completed advantage FT n_queries=${n_queries} seed=${seed}: ${save_path} ---"
      continue
    fi
    echo "--- run advantage FT n_queries=${n_queries} seed=${seed} $(date -Is) ---"
  ...
  echo "=== advantage FT n_queries=${n_queries} seed_grow finished $(date -Is) ==="
done

echo "=== traced AAV standardized-advantage FT run finished $(date -Is) ==="
