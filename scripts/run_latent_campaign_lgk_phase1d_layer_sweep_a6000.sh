...
SEED_END="${SEED_END:-5}"
LOG_PREFIX="${LOG_PREFIX:-/tmp/latent_campaign_lgk_phase1d}"

declare -a LAYERS=("1" "2" "4" "6")

for LAYER in "${LAYERS[@]}"; do
  PHASE="phase1d_layer_${LAYER}"
  LOG_PATH="${LOG_PREFIX}_layer_${LAYER}.log"
  echo "[latent-phase1d] phase=${PHASE} layer=${LAYER} gpu_uuid=${GPU_UUID} seeds=${SEED_START}-${SEED_END}"
