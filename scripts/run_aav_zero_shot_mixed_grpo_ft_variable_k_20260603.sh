...
BASE="${BASE:-outputs/aav_zero_shot_evodiff_ft_grpo_mixed_k4_variable_k_kl2_trace_batch64_20260603}"
TASK="${TASK:-AAV}"
...
echo "=== traced AAV mixed-mask GRPO FT run started $(date -Is) ==="
echo "BASE=${BASE}"
...
for n_queries in ${BUDGETS}; do
  ...
  echo "=== mixed GRPO FT n_queries=${n_queries} started $(date -Is) ==="
  for seed in ${SEEDS}; do
    save_path="${out_dir}/${TASK}/seed_${seed}.pkl"
    if seed_complete "${save_path}"; then
      echo "--- skip completed mixed GRPO FT n_queries=${n_queries} seed=${seed}: ${save_path} ---"
      continue
    fi
    echo "--- run mixed GRPO FT n_queries=${n_queries} seed=${seed} $(date -Is) ---"
  ...
  echo "=== mixed GRPO FT n_queries=${n_queries} finished $(date -Is) ==="
done

echo "=== traced AAV mixed-mask GRPO FT run finished $(date -Is) ==="
